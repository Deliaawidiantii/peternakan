...
export class KegiatanService {
  ...
  getKegiatan(status?: string): Observable<any> {
    const url = status
      ? `${this.apiUrl}/kegiatan?status=${status}`
      : `${this.apiUrl}/kegiatan`;
    return this.http.get(url, { headers: this.getHeaders() });
  }

  // Get detail kegiatan by ID
  getKegiatanById(id: number): Observable<any> {
    return this.http.get(`${this.apiUrl}/kegiatan/${id}`, {
  ...
  uploadLaporan(id: number, formData: FormData): Observable<any> {
    ...
    return this.http.post(
      `${this.apiUrl}/kegiatan/${id}/upload-laporan`,
      formData,
      { headers },
    );
  }
  ...
  selesaikanKegiatan(id: number): Observable<any> {
    return this.http.post(
      `${this.apiUrl}/kegiatan/${id}/selesai`,
      {},
      { headers: this.getHeaders() },
    );
  }
  ...
  getStatistik(): Observable<any> {
    return this.http.get(`${this.apiUrl}/kegiatan/statistik`, {
      headers: this.getHeaders(),
    });
  }

  // Request kegiatan baru
  requestKegiatan(data: any): Observable<any> {
    return this.http.post(`${this.apiUrl}/kegiatan`, data, {
      headers: this.getHeaders(),
    });
  }
}
